...
const validator = {
  /* 회원가입 유효성 검사
    1. 필수 데이터(아이디,비밀번호,비밀번호 확인, 회원명)
    2. 아이디
      1) 자리수 제한(8~20)
      2) 문자의 종류 제한(알파벳, 숫자) >> 정규표현식
      3) 중복 아이디 체크
    3. 비밀번호
      1) 자리수 제한(8자 이상)
      2) 복잡성 체크
        - 1개이상 알파벳, 숫자, 특수 문자로 구성된 비밀번호
      3) 비밀번호 확인이 일치하는 경우
  */
  ...
  async joinValidator(req, res, next) {
    try {
      ...
      /* 2. 아이디 유효성 체크 : (자리수8~20) + 알파벳+숫자
      memId 숫자 + 알파벳이 아닌 문자가 포함되어 있으면 X 
      /[^a-z0-9]/i    */
      const memId = data.memId;
      if (memId.length < 8 || memId.length > 20 || /[^a-z0-9]/i.test(memId)) {
        throw new Error(
          "아이디는 8~20자리 사이의 알파벳과 숫자로 구성해 주세요"
        );
      }
      ...
      /* 비밀번호 : 자리수(8이상)
        반드시 알파벳, 숫자, 특수문자가 포함
        /[^a-z]/i : a-z 아닌 문자(숫자 + 특수문자가 들어가 있으면 트루 >> 정확하게 알파벳 존재 여부를 확인 못함 >> 알파벳이 없어도 ture로 유효성 검사 성공)
      ...
      if (
        ...
        throw new Error(
          "비밀번호는 8자리 이상 알파벳, 숫자, 특수문자로 구성해 주세요."
        );
      }
      // 비밀번호 확인 일치 여부
